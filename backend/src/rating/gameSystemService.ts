...
export interface Quest {
  id: string;
  title: string;
  type: 'solve' | 'streak' | 'accuracy' | 'earn_xp' | 'consecutive' | 'perfect';
  target: number;
  current: number;
  completed: boolean;
  xpReward: number;
...
export const updateQuests = async (
  userId: number,
  client: PoolClient,
  action: 'attempt' | 'solve' | 'streak' | 'earn_xp',
  data?: { isCorrect?: boolean; xpEarned?: number; consecutiveCorrect?: number }
