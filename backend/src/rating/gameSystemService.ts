 ...
 */
export const checkAndRepairStreak = async (userId: number, client: PoolClient): Promise<{ repaired: boolean; newStreak: number; newTokens: number }> => {
  ...
  if (diff === 2) {
    if (tokens >= 15 && !user.streak_repaired) {
      ...
      await client.query(
        'UPDATE users SET tokens = $1, streak_repaired = TRUE, last_active_date = $2 WHERE id = $3',
        [tokens, lastActive, userId] // last_active_date를 어제나 그저께 상태로 둠으로써 오늘 풀면 정상 누적되게 처리
      );
    } else if (diff > 1) {
      // 복구 불가 혹은 토큰 부족 -> 스트릭 초기화 (0으로 리셋, 오늘 풀면 1이 됨)
      streak = 0;
      await client.query(
        'UPDATE users SET streak = $1, streak_repaired = FALSE WHERE id = $2',
        [streak, userId]
      );
    }
  } else if (diff > 2) {
...
export const handleDailyReset = async (userId: number, client: PoolClient, todayStr?: string): Promise<any> => {
  ...
  if (lastActive !== today) {
    ...
    await client.query(
      'UPDATE users SET streak_repaired = FALSE, quests = $1 WHERE id = $2',
